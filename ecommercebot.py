...
os.environ["LANGCHAIN_API_KEY"] = "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"
...
@traceable(project_name="ecommerce_support_chatbot")
...
st.set_page_config(page_title="Ecommerce Support Chatbot", layout="centered")
...
if not st.session_state["product_problem"]:
    ...
    product_problem_input = st.text_area("What issue are you facing with your order or product?")
    if st.button("Submit Issue"):
        if product_problem_input.strip():
            st.session_state["product_problem"] = product_problem_input.strip()
            st.success(f"Issue noted: {st.session_state['product_problem']}")
        else:
            st.warning("Please describe your issue.")
else:
    st.subheader(f"Issue: {st.session_state['product_problem']}")
